...
if [ -s missing_variants.bim.txt ]; then
    ...
    plink --bfile tgt_intersect --bmerge tgt_missing --make-bed --out tgt_combined
else
...
if diff <(awk '{print $4}' "$OUTPUT_PREFIX".bim) <(awk '{print $4}' "${REF_DATA}.bim") > /dev/null; then
      echo "Variant Order Checked: Positions are identically ordered. Proceeding ..."

      awk 'NR==FNR { col2[FNR]=$2; next } { $2 = col2[FNR]; print }' "${REF_DATA}.bim" "$OUTPUT_PREFIX".bim > "$OUTPUT_PREFIX".tmp
      mv "$OUTPUT_PREFIX".tmp "$OUTPUT_PREFIX".bim
