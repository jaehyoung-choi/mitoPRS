...
def main():
    ...
    XGB_PATH = "model/0.15_xgb_SCZ_model.pkl"
    ENET_PATH = "model/0.3_scz_enet_model.pkl"
    ...
    X_new, y_new, full_names, identifiers = utils.load_data_split(
        f"{args.ext_feature}_xgb.geno", args.ext_cov, f"scz_{args.train_names}_xgb.names", args.ext_label
    )
    ...
    X_new, y_new, full_names, identifiers = utils.load_data_split(
        f"{args.ext_feature}_enet.geno", args.ext_cov, f"scz_{args.train_names}_enet.names", args.ext_label
    )
    ...
    results['SCZ_mitoPRS_XGB'] = xgb_probs
    results['SCZ_mitoPRS_ENet'] = enet_probs

    # Loading other scores for appending
    csx_df = pd.read_csv(f"{args.out_prefix}.scz.csx.profile", sep=r'\s+', usecols=['FID', 'IID', 'SCORE']).rename(columns={'SCORE': 'SCZ_mitoPRS_PRSCSx'})
    ice_df = pd.read_csv(f"{args.out_prefix}.scz.all_score", sep=r'\s+', usecols=['FID', 'IID', 'Pt_0.42']).rename(columns={'Pt_0.42': 'SCZ_mitoPRS_PRSice'})
    pc_df = pd.read_csv(args.ext_cov, sep=r'\s+', usecols=['FID', 'IID', 'PC1', 'PC2', 'PC3', 'PC4', 'PC5'])

    # 7. Export
    out_file = os.path.join(output_dir, f"{args.out_prefix}_scz_predictions.csv")
